...
#------------------------------------------------------------
# API Wrapper script to deploy ARO cluster with managed identities
#------------------------------------------------------------
# Set your default once per session
AZ_API_VERSION="2022-04-01"

# Smart wrapper: appends --api-version only if the command supports it.
azv() {
  # Handle az rest by appending ?api-version=... to the URL if missing
  if [[ "$1" == "rest" ]]; then
    shift
    local method url args=()
    while (( "$#" )); do
      case "$1" in
        --url)
          url="$2"
          ...
          if [[ "$url" != *"api-version="* ]]; then
            if [[ "$url" == *"?"* ]]; then url="${url}&api-version=${AZ_API_VERSION}"
            else url="${url}?api-version=${AZ_API_VERSION}"
            fi
          fi
          args+=(--url "$url"); shift 2; continue
          ;;
        *) args+=("$1"); shift;;
      esac
    done
  ...
  # For normal commands, append --api-version only if supported
  if command az "$@" -h 2>&1 | grep -q -- '--api-version'; then
